...
public class OVRPlayerController : MonoBehaviour
{
    ...
    private bool TouchPadButtonPressed = false;
    ...
    public virtual void UpdateMovement()
    {
        ...
        if (EnableLinearMovement)
        {
            ...
            // Gear VR uses a touchpad, but we can simulate a dpad by making the user click after touching
            if (OVRInput.Get(OVRInput.Button.PrimaryTouchpad))
            {
                TouchPadButtonPressed = true;

                Debug.Log("Touchpad pressed DOWN");
            }
            else if (OVRInput.GetUp(OVRInput.Button.PrimaryTouchpad))
            {
                TouchPadButtonPressed = false;
                Debug.Log("Touchpad pressed UP");
            }

            // Touchpad is being pressed or held
            if (TouchPadButtonPressed)
            {
                Vector2 touchPosition = OVRInput.Get(OVRInput.Axis2D.PrimaryTouchpad);
                Debug.Log("Touchpad pos: " + touchPosition);

                dpad_move = true;
                if (touchPosition.x < -0.3) moveLeft = true;
                if (touchPosition.x > 0.3) moveRight = true;
                if (touchPosition.y < -0.3) moveBack = true;
                if (touchPosition.y > 0.3) moveForward = true;
            }
            else
            {
                moveLeft = false;
